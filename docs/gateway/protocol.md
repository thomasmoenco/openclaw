...
<AccordionGroup>
  ...
  <Accordion title="Agent and workspace helpers">
    - `agents.list` returns gateway-visible agent entries, including effective model/runtime metadata and optional semantic `kind` (`agent` or `system`). The required legacy `defaultId` remains populated during rolling upgrades: it is the sole agent, a retained migration owner, or the first configured agent when an explicit multi-agent fleet has no ambient owner. New clients use the additive `ownership` (`sole`, `legacy`, or `explicit`) and `selectionRequired` fields instead of treating that compatibility value as authoritative; old clients therefore retain their pre-upgrade first-entry behavior. The initial snapshot exposes the same fields beside its required legacy `defaultAgentId` and `mainSessionKey`. Clients advertise the `agent-kind` handshake capability to receive the complete typed roster; clients without it keep the legacy selector-safe roster without system rows. Kind-aware clients exclude `system` rows from ordinary selectors while retaining them in diagnostic views. Older v4 gateways may omit `kind` and the ownership fields.
